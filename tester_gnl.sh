...
if [ "$compare_flag" = true ]; then
    echo -e "${BLUE}Compilando e testando com objs...${NC}"
    ...
    echo -e "\n${BLUE}Compilando com objsbonus e testando com -bonus...${NC}"
    ...
    echo -e "${BLUE}Compilando com objsbonus...${NC}"
    ./compile.sh $buffer -bonus
    if [ $? -eq 0 ]; then
    ...
    echo -e "${BLUE}Compilando e testando com -bonus...${NC}"
    ./compile.sh $buffer
    if [ $? -eq 0 ]; then
    ...
    echo -e "${BLUE}Compilando e testando sem -bonus...${NC}"
    ./compile.sh $buffer
    if [ $? -eq 0 ]; then
